...
public:
	...
	XSFDriver() throw()
		: hDriver(0), lpif(0), m_samplerate(samplerate)
	{
	}
	~XSFDriver()
	{
	...
	bool Load(xsfc::TString fn)
	{
		...
		if (lpif->dwInterfaceVersion >= 5)
		{
			lpif->SetExtendParamVoid(1, &loInterpolationLevel);
			lpif->SetExtendParamVoid(2, &loResampler);
			lpif->SetExtendParamVoid(3, &m_samplerate);
			lpif->SetExtendParamVoid(4, &dwDisableSurround);
			lpif->SetExtendParamVoid(5, &dwReverseStereo);
		}
	...
	void Start()
	{
		len_smp = MulDiv(length_in_ms, m_samplerate, 1000);
		fad_smp = MulDiv(fade_in_ms, m_samplerate, 1000);
		seek_pos = 0;
		seek_kil = 0;
		seek_req = 0;
		SeekTop();
		...
		while (pos < bufsize)
		{
			...
			if (detectSilence || skipSilenceOnStartSec)
			{
				...
				for (ofs = 0; ofs < remain; ofs++)
				{
					...
					if (silence)
					{
						if (++detectedSilenceSample >= m_samplerate)
						{
							detectedSilenceSample -= m_samplerate;
							detectedSilenceSec++;
	...
	int FillBuffer(void *pbuf, unsigned bufbytes, unsigned *writebytes, unsigned isdecoder)
	{
		...
		unsigned cur = MulDiv(cur_smp, 1000, m_samplerate);
		int len = bufsize;
		Gen(pbuf, bufsize);
	...
	{
		unsigned bufsize = bufbytes >> blockshift;
		unsigned seek_smp = MulDiv(seek_pos, m_samplerate, 1000);
...
protected:
	static DWORD __stdcall PlayWorkerThread(LPVOID argp)
	{
		try
		{
			...
			do
			{
				...
				unsigned cur = MulDiv(xsfdrv.cur_smp, 1000, xsfdrv.m_samplerate);
				...
				if(mod.dsp_isactive())
					len = mod.dsp_dosamples((xsfsample_t *)(buffer), len, xsfdrv.samplebits, xsfdrv.ch, xsfdrv.m_samplerate);
	...
	static int __cdecl play(const in_char *fn)
	{
		try
		{
			...
			maxlatency = mod.outMod->Open(xsfdrv.m_samplerate, xsfdrv.ch, xsfdrv.samplebits, 0, 0);
			if (maxlatency < 0)
			{
				xsfdrv.Free();
				return 1;
			...
			mod.SetInfo(xsfdrv.m_samplerate * xsfdrv.samplebits * xsfdrv.ch / 1000, xsfdrv.m_samplerate / 1000, xsfdrv.ch, 0);
			mod.SAVSAInit(maxlatency, xsfdrv.m_samplerate);
			mod.VSASetInfo(xsfdrv.m_samplerate, xsfdrv.ch);
...
static void * __cdecl winampGetExtendedRead_opensub(xsfc::TString fn, int *size, int *bps, int *nch, int *srate)
{
	try
	{
		...
		if (nch) *nch = pDrv->ch;
		if (srate) *srate = pDrv->m_samplerate;
		return pDrv.Detach();
	}
	catch (xsfc::EShortOfMemory ex)
	{
