...
public:
	...
	void SetLong(const wchar_t *name, const long value) throw()
	{
		try
		{
			xsfc::TWin32::SetPrivateProfile(sIniPath, xsfc::TString(XSFDRIVER_SIMPLENAME), name, xsfc::TString(value));
	...
	long GetLong(const wchar_t *name, const long defaultvalue = 0) throw()
	{
		try
		{
			xsfc::TString value = xsfc::TWin32::GetPrivateProfile(sIniPath, xsfc::TString(XSFDRIVER_SIMPLENAME), name, xsfc::TString(defaultvalue));
			if (value[0]) return value.GetLong();
		}
		catch (xsfc::EShortOfMemory e)
		{
		}
...
#endif
		case IDC_INTERPOLATION: case IDC_RESAMPLER: case IDC_DISABLE_SURROUND: case IDC_REVERSE_STEREO:
			if (initialized)
			{
				CFGUpdate(pcfg, hwndDlg);
				CFGSave(pcfg);
